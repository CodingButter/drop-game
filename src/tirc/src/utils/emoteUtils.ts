...
export const fetchBTTVChannelEmotes = async (
  ...
  try {
    ...
    const data = await response.json();
    return [...(data.channelEmotes || []), ...(data.sharedEmotes || [])].map((emote: any) => ({
      id: emote.id,
      name: emote.code,
      urls: {
        "1x": `https://cdn.betterttv.net/emote/${emote.id}/1x`,
    ...
    return Object.values(data.sets)
      ...
      .map((emote: any) => ({
        ...
        urls: {
          ...
          "3x": emote.urls["4"] || emote.urls["2"] || emote.urls["1"] || "",
        },
      }));
  } catch (error) {
...
export const fetchTwitchEmotes = async (
  ...
  try {
    ...
    return emotesData.data.map((emote: any) => ({
      id: emote.id,
      name: emote.name,
      urls: {
        "1x": emote.images.url_1x,
        "2x": emote.images.url_2x,
        "3x": emote.images.url_4x,
      },
    }));
  } catch (error) {
